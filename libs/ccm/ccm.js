...
(() => {
  ...
  const Datastore = function () {
     ...
     */
    this.source = () => {
      return {name: that.name, url: that.url, db: that.db}
    };
  ...
  const ccm = {
    ...
    load: function () {
      ...
      return new Promise((resolve, reject) => {
        ...
        args.forEach((resource, i) => {
          ...
          // timeout check
          let timeout;
          ccm.timeout && window.setTimeout(() => timeout === undefined && (timeout = true) && error('timeout'), ccm.timeout);
          ...
          function loadJS() {
            ...
            window.ccm.files['#' + filename] = window.ccm.files['#' + filename] ? window.ccm.files['#' + filename] + 1 : 1;
          ...
          function loadJSON() {
            ...
            function jsonp() {
              ...
              const callback = 'callback' + ccm.helper.generateKey().replaceAll('-', '');
    ...
    instance: async (component, config) => {
      ...
      config.beforeCreation && await config.beforeCreation(config, ccm.helper.clone(component));
      ...
      config.afterCreation && await config.afterCreation(config, ccm.helper.clone(component));
      ...
      function createInstance() {
        ...
        delete config.parent;         // reference of parent ccm instance
        instance.children = {};                                          // reference to children instances
        instance.root = config.root;
        delete config.root;           // instance root element
        ...
        };             // each instance must have a init method
        if (instance.parent) {
          ...
          function find(obj) {
            ...
            for (const key in obj)
              if (obj.hasOwnProperty && obj.hasOwnProperty(key)) {
                ...
                if (ccm.helper.isInstance(value) && key !== 'parent' && !ccm.helper.isProxy(value)) {
    ...
    store: (config = {}) => new Promise((resolve, reject) => {
      ...
      ccm.helper.solveDependency(config.local).then(result => {
        config.local = result;
